...
int64_t volatile ui64_lptim3=0;
...
uint64_t Get_Absolute_Ms64(void) {
    uint64_t base;
    uint32_t ticks;
...
for (int ii =0; ii<VbAT_Nb_MES ; ii=ii+1){
   HAL_ADC_Start(&hadc1);
   HAL_ADC_PollForConversion(&hadc1, HAL_MAX_DELAY);
   adc_res += HAL_ADC_GetValue(&hadc1);
}
u32_vbat = ((adc_res) * 4 * 33000/ VbAT_Nb_MES)/16384+5; 
/*
   HAL_ADC_Start(&hadc1);
   HAL_ADC_PollForConversion(&hadc1, HAL_MAX_DELAY);
   adc_value1 = HAL_ADC_GetValue(&hadc1);
   ...
   adc_value3 = HAL_ADC_GetValue(&hadc1);
    ...
    u32_vbat = ((adc_value1 + adc_value2 + adc_value3) * 4 * 33000/3)/16384+5; 
// correction diode protection for Vbat
*/
