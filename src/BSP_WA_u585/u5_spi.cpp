...
LCD_Status_t LCD_Clear(LCD_Handle_t *p_hlcd)
{
    ...
    p_hlcd->status = SPI_SendCommand(p_hlcd, cmd);
    if (p_hlcd->status != LCD_OK) {
        return p_hlcd->status;
    }
...
#define LCD_COL_TOTAL_SIZE    (LCD_CMD_HEADER_SIZE + (LCD_HEIGHT/8))
...
#if DEBUG
   RTT_vprintf_cr_T_F_L(LCD_BuildDMABuffer_32_rot, DL_display, "Dma X:%d-%d, Y:%d-%d, size %d, col min : %d", 
