...
EXTI_HandleTypeDef OS_RAM h_col1_exti_13, h_col2_exti_14, h_col3_exti_10, h_col4_exti_15, h_col5_exti_2, h_col6_exti_1;
...
//   OS_TASKEVENT_Set( &TDB48X, EV_DBx_PB_PA0);
...
{
   HAL_EXTI_IRQHandler(&h_col6_exti_1);     
}  

void EXTI2_IRQHandler(void)
{
   HAL_EXTI_IRQHandler(&h_col5_exti_2);     
}

void EXTI10_IRQHandler(void)
{
   HAL_EXTI_IRQHandler(&h_col3_exti_10);     
}

void EXTI15_IRQHandler(void)
{
   HAL_EXTI_IRQHandler(&h_col4_exti_15);     
}

void EXTI13_IRQHandler(void)
{
   HAL_EXTI_IRQHandler(&h_col1_exti_13);     
}

void EXTI14_IRQHandler(void)
{
   HAL_EXTI_IRQHandler(&h_col2_exti_14);     
}
...
// init interrupts
  /* EXTI interrupt init*/
   ...
   for (uint32_t ii = 0; ii < KB_MAX_KEY; ii++){
      ...
      p_kbd->key_press_time[ii] = 0;
   }
   p_kbd->ValidCount = KB_PRESS_COUNT;
...
//   EXTI->PR1 = 0xffff; // not ok for u5
   ...
   switch(mode){
      ...
      case    kb_scrut_int:
         ...
         p_kbd->ValidCount = KB_PRESS_COUNT-1;
         break;
      ...
      for (uint32_t i_col = 0; i_col < KB_COL; i_col++){
         ...
         { // key pressed
            ...
            for (uint32_t ii = 0; ii < KB_MAX_KEY; ii++)
            {
               ...
               { 
                  p_kbd->key_press_time[ii] +=1;
                  p_kbd->key_release_time[ii] =0;
                  key_pressed_counting = true;
                  if (p_kbd->key_press_time[ii] == p_kbd->ValidCount)
                  {
         ...
         { // key not pressed
            ...
            for (uint32_t ii = 0; ii < KB_MAX_KEY; ii++){
               if ((key_value_act == p_kbd->key_value[ii])&&
                  (p_kbd->key_press_time[ii] >= KB_PRESS_COUNT)){ 
                     p_kbd->key_release_time[ii] += 1;
                     if (p_kbd->key_release_time[ii] > KB_RELEASE_COUNT){
                  ...
                  {
                     if (p_kbd->key_press_time[ii] >= KB_PRESS_COUNT){
                        shift += 8;
                        if (8 == shift) p_kbd->dts.key1 = keyb_corr(p_kbd->key_value[ii]);
                        if (16 == shift) p_kbd->dts.key2 = keyb_corr(p_kbd->key_value[ii]);
                        if (24 == shift) p_kbd->dts.key3 = keyb_corr(p_kbd->key_value[ii]);
...
// à modifier : faire un tableau avec 100 entrées !
// ----------------------------------------------------------------------------
{
...
uint32_t key_DB_to_DM_old(uint32_t key){
...
//   Convert DMCP key codes to row/column positions
// ----------------------------------------------------------------------------
const uint8_t dmcp_position_d[110]={
...
uint32_t key_DB_to_DM(uint32_t key){
   const size_t max = sizeof(dmcp_position_d) / sizeof(dmcp_position_d[0]) - 1;
   if (key > max) return 0;
   else return dmcp_position_d[key];
}
