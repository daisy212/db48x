...
bool RTC_Config_1024_Granularity(bool force_reset)
{
 HAL_PWR_EnableBkUpAccess();
    ...
    if ( force_reset
      ||(HAL_RTCEx_BKUPRead(&hrtc, RTC_BKP_DR0) != RTC_INIT_MARKER))
    {
        ...
        if (HAL_RCCEx_PeriphCLKConfig(&PeriphClkInitStruct) != HAL_OK) return false;
...
/**
  ...
  * @retval None
  */
void RTC_Config_1024_Granularity_old(void)
{
  RCC_OscInitTypeDef RCC_OscInitStruct = {0};
  RCC_PeriphCLKInitTypeDef PeriphClkInitStruct = {0};
...
__HAL_RCC_PWR_CLK_ENABLE();  // ????
  HAL_PWR_EnableBkUpAccess();
   ...
   __HAL_RCC_RTCAPB_CLK_ENABLE();
// Check if RTC is already initialized by reading INITS flag
    if (READ_BIT(RTC->ICSR, RTC_ICSR_INITS) == RTC_ICSR_INITS)
    {
        // RTC already initialized, just attach the handle
        hrtc.Instance = RTC;
